...
class Check extends React.Component {
  ...
  getTableList = () => {
    if (
      this.params["requestStartTime"] &&
      this.params["requestEndTime"] &&
      this.params["requestStartTime"] > this.params["requestEndTime"]
    ) {
    ...
    getCheckList(this.params)
      ...
      .then((dataObject) => {
        let tableList = [];
        dataObject.result &&
          dataObject.result.list.forEach((item) => {
            let obj = item;
            obj.time = dateUtils.formatStandardNumberDate(item.time) || "";
            obj.info.start =
              dateUtils.formatStandardNumberDate(item.info.start, true) || "";
            tableList.push(obj);
          });
        this.setState({
          tableList,
  ...
  changeTab = (key) => {
    if (this.state.tabIndex === 3 || this.state.tabIndex === 4) {
      if (key === 3 || key === 4) {
        //检查无误”与“检查错误”tab页互相切换时，筛选条件，且带出搜索结果
        this.setState(
          {
