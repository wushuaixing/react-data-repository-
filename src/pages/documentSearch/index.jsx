...
class Check extends React.Component {
  ...
  handleSearch = (e) => {
    ...
    setFieldsValue({
      whole: get("whole").trim().replace(/\s+/g, " "),
      ah: get("ah").trim(),
  ...
  render() {
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (record) => {
          const temp = (
            ...
            >
              {filters.blockNullData(record.title, "—")}
            </span>
          );

          return (
            <Link
              to={`/documentDetail/${record.wenshuId}/${record.wid}/${
      ...
      },
      {
        title: "案由",
        render(record) {
          return <span>{filters.blockNullData(record.reason, "—")}</span>;
        },
      },
      {
        title: "案件类型",
            ...
            <div className="yc-detail-content">
              <div className="yc-search-line document-search">
                ...
                >
                  ...
                  {documentSearchRecords &&
                    documentSearchRecords.length > 0 && (
                      <SearchRecord
  ...
  <div className="record-content">
    ...
    {props.records.map((item, index) => {
      return (
        <span
          key={`records${index}`}
          onClick={() => props.handClick(item, index)}
        >
          {item}
        </span>
      );
    ...
    <Popover content={"清空最近搜索记录"}>
      <span className="del_icon">
        <img
          src={props.isHover ? RecordsDelHover : RecordsDel}
          onClick={props.clearRecords}
          onMouseOver={() => props.handIconHover(true)}
          onMouseLeave={() => props.handIconHover(false)}
          alt=""
        />
      </span>
    </Popover>
