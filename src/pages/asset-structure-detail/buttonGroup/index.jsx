...
class ButtonGroup extends Component {
  ...
  get checkButtons() {
    ...
    return {
      err: (
        <Button
          onClick={this.handleErrorModal.bind(this, "err")}
          key="0"
          style={{ marginLeft: 10 }}
        >
          {"检查有误"}
      ...
      noErr: (
        <Button
          onClick={this.handleNoErr.bind(this)}
          key="1"
          style={{ marginLeft: 10 }}
        >
          {"检查无误"}
      ...
      modify: (
        <Button
          onClick={this.handleErrorModal.bind(this, "modify")}
          key="5"
          style={{ marginLeft: 10 }}
        >
          {"修改错误原因"}
    ...
    return [
      ...
      },
      {
        btns: [
          toAffirm,
          checkButtons["err"],
          !toAffirm ? checkButtons["back"] : null,
        ],
      },
      {
      ...
      <div className="detail-buttonGroup">
        {(() => {
          switch (role) {
            ...
            case "check":
              if (enable) {
                switch (status) {
                  case 2:
                    return (
                      <Fragment>{this.checkButtonTextArray[1].btns}</Fragment>
                    ); //未检查
                  case 3:
                    return (
                      <Fragment>{this.checkButtonTextArray[2].btns}</Fragment>
                    ); //检查无误
                  case 4:
                    return (
                      <Fragment>{this.checkButtonTextArray[3].btns}</Fragment>
                    ); //检查有误
                  case 5:
                    return (
                      <Fragment>{this.checkButtonTextArray[4].btns}</Fragment>
                    ); //已修改
                  default:
                    return null;
                }
              } else {
                return <Fragment>{this.checkButtonTextArray[0].btns}</Fragment>;
              }
            case "newpage-check":
              if (enable) {
                switch (associatedStatus) {
                  case 2:
                    return (
                      <Fragment>
                        {checkButtons["err"]} {checkButtons["noErr"]}
                      </Fragment>
                    ); //未检查
                  case 3:
                    return (
                      <Fragment>
                        {checkButtons["err"]} {checkButtons["close"]}
                      </Fragment>
                    ); //检查无误
                  case 4:
                    return (
                      <Fragment>
                        {checkButtons["modify"]} {checkButtons["noErr"]}
                      </Fragment>
                    ); //检查有误
                  case 5:
                    return (
            ...
            case "newpage-other":
              return <Fragment>{checkButtons["close"]}</Fragment>;
            default:
              return <Fragment>{checkButtons["close"]}</Fragment>;
          }
        })()}
...
const OnlyMarkButton = ({ handleChange, value }) => {
  ...
  return (
    <div className="buttonGroup-onlyMark">
      <Checkbox onChange={handleChange} name="onlyThis" checked={_value}>
        仅标记本条
      </Checkbox>
    </div>
  );
