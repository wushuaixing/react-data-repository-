            ...
            <div className='yc-component-assetStructureDetail basic_info'>
                ...
                <div className="yc-component-assetStructureDetail_body">
                    <ul>
                        ...
                        {
                            !hasAuto && associatedAnnotationId &&       //如果结构化检查记录存在有自动标注的数据，关联标注不显示
                            <Item title='关联标注：'>
                                ...
                                <div className="records">
                                    {
                                        records.map((item, index) => {
                                            return (<p key={`record${index}`}>
                                                <RecordsItem record={item} index={index}
...
const RecordsItem = (props) => {
    ...
    return (
        <Fragment>
            {`${dateUtils.formatStandardNumberDate(record.time)} ${record.name}${userType === 1 ? '检查' : ''}`} <span
            className={classType}>{temp}</span>
            {
                record.msg === '自动标注' ?
