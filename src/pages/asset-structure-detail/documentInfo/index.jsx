                            ...
                            <div>
                                {
                                    enable ?
                                        ...
                                        : <Radio.Group value={wsFindStatus}
                                                       name="wsFindStatus"
                                        ...
                                        <div>
                                            {
                                                enable ?
                                                    ...
                                                    <span>
                                                        <Checkbox name="wsInAttach"
                                                            onChange={this.handleChange}
                                                            checked={this.wsInAttach}
                                                            disabled={enable}>详情见资产拍卖附件
                                                        </Checkbox>
                                                    </span>
                                            }
                                        </div>
...
const DocumentLinkInput = (props) => {
    ...
    return (
        <div className='doucment_info_inputItem'>
            {
                enable ?
                    (attr === 'wsUrl' ? (linkSpan(value.value) || '-') : (value.value || '-'))
                    : <Input
            ...
            }
            {
                (num === index + 1) && num < 3 && !enable ?
                    <img src={ICONADD} style={{width: 18, height: 18, marginLeft: 12}} alt=" "
                         className="icon_hover_pointer" onClick={props.handleAddClick}/> :
                    null
            }
            {
                num > 1 && !enable ?
                    <img src={ICONDEL} style={{width: 18, height: 18, marginLeft: 12}} alt=" "
                         className="icon_hover_pointer" onClick={props.handleDeleteClick}/> :
                    null
            }
        </div>
    )
