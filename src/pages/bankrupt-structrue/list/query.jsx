...
class ListQuery extends React.Component {
  ...
  getStructuredPersonnelTypeList({ chineseLetter = [], digit = [] }) {
    ...
    const normalList = (ary) => {
      ...
      ary.forEach((item) => {
        ...
        tempAry[field]
          ? tempAry[field].push(toFill(item))
          : (tempAry[field] = [toFill(item)]);
      });
    ...
    return [
      ...
      ...normalList(chineseLetter),
      { id: "#", array: digit.map((item) => toFill(item)) },
    ];
  }
  ...
  handleSearch = (e) => {
    ...
    const params = simply
      ? getFieldsValue(["title", "publishStartTime", "publishEndTime"])
      : getFieldsValue();
    Object.keys(params).forEach((i) => {
      if (typeof params[i] === "object" && /Time$/.test(i) && params[i])
        params[i] = params[i].format("YYYY-MM-DD");
    });
  ...
  render() {
    ...
    const publishTime = (
      <div style={{ display: "inline-block" }} key="publishTime">
        <Form.Item
          label="发布日期"
          key="publishStartTime"
          className="list-query-wrapper-form_before end-time-after"
        >
          {getFieldDecorator("publishStartTime", { initialValue: null })(
            <DatePicker
              placeholder="起始日期"
              disabledDate={(val) =>
                this.disabledStartDate(val, "publishEndTime")
              }
              style={{ width: 120 }}
        ...
        <Form.Item
          label="至"
          key="publishEndTime"
          className="list-query-wrapper-form_link"
        >
          {getFieldDecorator("publishEndTime", { initialValue: null })(
            <DatePicker
              placeholder="截止日期"
              disabledDate={(val) =>
                this.disabledEndDate(val, "publishStartTime")
              }
              style={{ width: 120 }}
              getCalendarContainer={(node) => node.offsetParent}
            />
    ...
    const updateTime = (
      <div style={{ display: "inline-block" }} key="updateTime">
        <Form.Item
          label="更新时间"
          key="updateStartTime"
          className="list-query-wrapper-form_before end-time-after"
        >
          {getFieldDecorator("updateStartTime", { initialValue: null })(
            <DatePicker
              placeholder="起始日期"
              disabledDate={(val) =>
                this.disabledStartDate(val, "updateEndTime")
              }
              style={{ width: 120 }}
              getCalendarContainer={(node) => node.offsetParent}
            />
        ...
        >
          {getFieldDecorator("updateEndTime", { initialValue: null })(
            <DatePicker
              placeholder="截止日期"
              disabledDate={(val) =>
                this.disabledEndDate(val, "updateStartTime")
              }
              style={{ width: 120 }}
              getCalendarContainer={(node) => node.offsetParent}
            />
    ...
    const Items = simply
      ? [title, publishTime]
      : [companyName, title, updater, publishTime, updateTime];
    ...
    return (
      <div className="list-query-wrapper">
        <Form
          ...
          className="list-query-wrapper-form"
        >
          {Items}
