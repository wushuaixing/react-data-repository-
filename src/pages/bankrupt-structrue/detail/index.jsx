...
class BankruptDetail extends React.Component {
  ...
  getItems = (field, placeholder = "请输入") => {
    ...
    const toHandle = (type, field, val) => {
      ...
      if (type === "add") {
        nextValues = values.concat(ranStr());
      }
      if (type === "sub") {
        nextValues = values.filter((i) => i !== val);
      }
      setFieldsValue({ [field]: nextValues });
    };
    // 获取 field 错误状态
    const getError = (field) =>
      getFieldError(field) ? getFieldError(field).join(",") : null;
    // input 变化校验
    ...
    return itemArray.map((item, index) => {
      ...
      return (
        <div
          key={index}
          className={`detail-item_input${err ? " detail-item_error" : ""}`}
        >
          {getFieldDecorator(_field, {
            onChange: (e) => toInputChange(e, _field, err),
          })(
            <Input
              placeholder={placeholder}
              style={{ width: 260 }}
              autoComplete="off"
              onBlur={(e) => onBlur(e, _field)}
            />
          )}
          {err && <span className="detail-item_input__error">{err}</span>}
          {itemArray.length === index + 1 && (
            <Icon
              className="detail-item_input__icon"
              type="plus-circle"
              theme="filled"
              onClick={() => toHandle("add", field, item)}
            />
          )}
          {itemArray.length > 1 && (
            <Icon
              className="detail-item_input__icon"
              type="minus-circle"
              onClick={() => toHandle("sub", field, item)}
            />
          )}
        </div>
      );
  ...
  setUserInfo = (source) => {
    const {
      form: { setFieldsValue: set, setFields },
    } = this.props;
    const {
      companyName: company,
      applicant: apply,
    ...
    const errorStr = [];
    const addValue = (array, field, checkField = "value") =>
      (array || []).forEach((i) => {
        if (i[checkField]) {
    ...
    set(data.field, () => {
      set(data.value);
      errorStr.forEach((i) =>
        setFields({ [i.field]: { value: i.value, errors: i.errors } })
      );
    });
  };
  // 获取 => 表单数据
  ...
  get ChangeStatus() {
    ...
    const {
      source: { companyName, applicant, publisher },
    } = this.state;
    ...
    const compareRes =
      compareArray(
        getAry(companyName, "bankruptcyCompanyName"),
        getAry(source.companyName)
      ) ||
      compareArray(getAry(applicant), getAry(source.applicant)) ||
      compareArray(getAry(publisher), getAry(source.publisher));
    return !compareRes;
  }
  ...
  toCheck = async (id) => {
    ...
    let result = "";
    if (res.code === 200)
      result = source.status === res.data ? "normal" : "error";
    else result = "warning";
    return result;
  };
  ...
  toSave = async () => {
    ...
    if (this.ChangeStatus)
      return message.error("当前页面未作修改，请修改后再保存");
    if (!source.companyName.length)
      return message.warning("请输入破产企业名称！");
    this.setState({ loading: true });
    // 校验当前数据状态
    ...
    if (idStatus !== "normal") {
      ...
      if (idStatus === "error")
        return message.error("该数据已被检查错误，请到待修改列表查看", 2, () =>
          history.go(-1)
        );
  ...
  toSaveNext = async (type) => {
    ...
    if (this.ChangeStatus && type === "modify")
      return message.error("当前页面未作修改，请修改后再保存");
          ...
          .then((res) => {
            if (res.code === 200) {
              if (res.data) {
                ...
                message.error(`${text}，为您跳转至未标记列表`, 2, () => {
        ...
        .then((res) => {
          if (res.code === 200) {
            if (res.data) {
              ...
              this.toWillBackModal(type === "modify" && "已修改完全部数据，");
          } else message.error(res.message);
        })
        .catch(() => message.error("服务繁忙，请稍后再试"))
        .finally(this.setState({ loading: false }));
          ...
          .then((res) => {
            if (res.code === 200) {
              if (res.data) {
                ...
                message.error("该数据已被处理，为您跳转至未标记列表", 2, () => {
                  history.push(`/index/bankrupt?approveStatus=0`);
                });
            } else message.error(res.message);
          })
  ...
  toWillBackModal = (text) => {
    ...
    const modal = Modal.success({
      ...
      title: [
        text || "已标记完全部数据，",
        <span style={{ color: "#016AA9" }} key="time">
          {secondsToGo}s
        </span>,
        " 后回到未标记列表",
      ],
            ...
            <div className="detail-content">
              <Item title="自动退回" hide={!(source.status === 2)}>
                <ul className="detail-content-item_ul">
                  {(autoReturn || {}).time && rule === "admin" ? (
                    <li>
                      <span className="li-span li-span_time">
                        {autoReturn.time}
                      </span>
                      {autoReturn.msg && (
                        <span className="li-span">{autoReturn.msg}</span>
                      )}
                      {autoReturn.bol && (
                        <span className="li-span" style={{ color: "#FB0037" }}>
                          有误
                        </span>
                      )}
                    </li>
              ...
              <Item title="角色信息" hide={rule !== "admin"}>
                <div style={{ display: "flex" }}>
                  <ItemList
                    title="破产企业："
                    style={{ maxWidth: 500, paddingRight: 120 }}
                  >
                    <ul className="detail-content-item_ul">
                      {(source.companyName || []).map(
                        ({ bankruptcyCompanyName: name, bol }) => (
                          <ItemTag key={name} text={name} tag={bol} />
                        )
                      )}
                  ...
                  <ItemList title="申请人：">
                    <ul className="detail-content-item_ul">
                      {(source.applicant || []).map(({ value: name, bol }) => (
                        <ItemTag key={name} text={name} />
                      ))}
                ...
                <ItemList title="破产管理人：">
                  <ul className="detail-content-item_ul">
                    {(source.publisher || []).map(({ value: name, bol }) => (
                      <ItemTag key={name} text={name} />
                    ))}
              ...
              >
                <div style={{ display: "flex", paddingBottom: 10 }}>
                  <ItemList
                    title="破产企业："
                    style={{ maxWidth: 500, paddingRight: 40 }}
                    required
                  >
                ...
                <ItemList title=" ">
                  ...
                  {source.status === 2 && [
                    <Button key="noError" onClick={this.toAffirm}>
                      信息无误
                    </Button>,
                    <Button
                      key="nextTag"
                      type="primary"
                      onClick={() => this.toSaveNext("modify")}
                    >
                      保存并修改下一条
                    </Button>,
                  ]}
...
export default Form.create({ name: "bankrupt_detail" })(
  withRouter(rule(BankruptDetail))
);
