    ...
    if (associatedAnnotationId) {
        switch (localStorage.getItem('userState')) {
            ...
            case '检查人员':
                ...
                autoMarkRoute = `/notFirstMark/check/${props.associatedAnnotationId}`;
                break;
            case '结构化人员':
                associatedAnnotationRoute = `/autoMark/structure/${associatedAnnotationId}/${associatedStatus}`;
                break;
            default:
                break;
        }
            ...
            <div className="yc-components-basicDetail_body">
                ...
                }
                {
                    props.records && props.records.length > 0 ?
                        <BasicDetailRow title={'结构化/检查记录'} content={props.records} autoMarkRoute={autoMarkRoute}/> : null
                }
            </div>
        </div>
    )
                                    ...
                                    props.content.map((record, index) => {
                                        return (
                                            <span className={'yc-components-assetStructureDetail_body-row_content'} key={index} style={index !== 0 ? { marginLeft: 113.4, display: 'inline-block', marginTop: 5 } : null}>
                                                <StructureRecord record={record} key={index} index={index} autoMarkRoute={props.autoMarkRoute}/>
                                            </span>
                                        )
